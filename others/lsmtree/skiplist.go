...
func (sl *SkipList) add(key []byte, val []byte, update bool) bool {
	sl.total++
	sl.maxLevel = int(math.Ceil(math.Log2(float64(sl.total)))) // 这个地方设置了maxLevel
	if sl.maxLevel == 0 {
		sl.maxLevel = 1
	}
...
func (sl *SkipList) seek(key []byte, oper operType) (*skipListNode, []*skipListNode) {
	...
	var prev []*skipListNode // 相当于update
...
func (sl *SkipList) String() string {
	...
	for i := 0; i < len(buf); i++ {
		...
		buf[i].WriteString(fmt.Sprintf("level %d: ", i))
	}
	node := sl.head
	for node != nil {
