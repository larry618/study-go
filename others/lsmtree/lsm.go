...
func (lsm *LSM) Levels() int {
	return len(lsm.manifest)
}
...
func (lsm *LSM) add(key []byte, val []byte) bool {
	...
	if eleSize+lsm.mem.Size() >= int(lsm.MemSyncSize) {
		lsm.imem = lsm.mem
		lsm.mem = NewSkipList()
		lsm.syncImem() // 把 immutable memory table 的信息存到硬盘上
	}
	lsm.mem.AddOrUpdate(key, val)
	return true
}
func (lsm *LSM) Get(key []byte) ([]byte, bool) {
	val, ok := lsm.mem.Get(key) // 先从内存中的 skipList 中查找  那imutable
	if ok {
...
func (lsm *LSM) syncImem() {
	...
	node := lsm.imem.head
	var minKey, maxKey []byte // 文件 file 中的最大值和最小值
	minKey = node.key
	size := 0 // 当前 file 已经存储的 size
	...
	for node != nil {
		...
		if eleBuf.Len() <= spare-prefixSize { // 当前block剩余空间足够
			...
			maxKeyMap[block] = node.key // 当前区块的最后一个key 也就是最大的
			size += prefixSize + eleBuf.Len()
		} else { // 当前block 剩余空间不够

			if prefixSize+int(unsafe.Sizeof(lsmKeyLenType(0)))+len(node.key) < spare { // 看看剩余空间能不能存下key
				// 剩余空间能存下key
				...
				eleBuf.Read(data)
				binary.Write(buf, binary.LittleEndian, data) // 把能装的都装进去
				block = size / int(lsm.BlockSize)
				maxKeyMap[block] = node.key // 当前区块的最后一个key 也就是最大的
				size += spare
				spare = int(lsm.BlockSize)
				...
				binary.Write(buf, binary.LittleEndian, make([]byte, spare)) // 存空的byte进去  填充空的数据 对齐
				size += spare
				spare = int(lsm.BlockSize) // 新起一个区块
				continue
			}
		}
		...
		if node.next[0] == nil {
			maxKey = node.key // 文件的最大值
		}
		node = node.next[0] // 存储下一个 key value
	}

	if size%int(lsm.BlockSize) > 0 { // 填充空的数据 对齐
	...
	// 更新 manifest  那之前的manifest呢  我怎么觉得只有一层啊
